...
public class VocabularyServiceImpl implements VocabularyService {
    ...
    public VocabularyDto getWordOfTheDay(String dateYyyyMmDd) {
        ...
        if (all.isEmpty()) throw new MyNotFoundException("Lüğətdə söz yoxdur");
    ...
    public VocabularyDto create(VocabularyCreateDto dto) {
        ...
        VocabularyEntity entity = VocabularyEntity.builder()
                .word(dto.getWord()).meaning(dto.getMeaning()).exampleSentence(dto.getExampleSentence()).level(level).build();
    ...
    public VocabularyDto update(Long id, VocabularyCreateDto dto) {
        ...
        LevelEntity level = levelRepository.findById(dto.getLevelId())
                .orElseThrow(() -> new MyNotFoundException("Level tapılmadı: " + dto.getLevelId()));
        entity.setWord(dto.getWord()); entity.setMeaning(dto.getMeaning()); entity.setExampleSentence(dto.getExampleSentence()); entity.setLevel(level);
        entity = vocabularyRepository.save(entity);
        return toDto(entity);
    }
    ...
    public void deleteById(Long id) {
        if (!vocabularyRepository.existsById(id)) throw new MyNotFoundException("Vocabulary tapılmadı: " + id);
        vocabularyRepository.deleteById(id);
    }

    private VocabularyDto toDto(VocabularyEntity e) {
        return VocabularyDto.builder().id(e.getId()).word(e.getWord()).meaning(e.getMeaning()).exampleSentence(e.getExampleSentence())
                .levelId(e.getLevel().getId()).levelName(e.getLevel().getName().name()).build();
    }
}
