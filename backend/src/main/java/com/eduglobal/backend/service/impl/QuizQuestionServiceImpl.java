...
public class QuizQuestionServiceImpl implements QuizQuestionService {
	...
	public List<QuizQuestionDto> findRandomByLevelId(Long levelId, int limit) {
		...
		return quizQuestionRepository.findRandomByLevelIdLimit(levelId, safeLimit).stream().map(this::toDto)
	...
	public QuizQuestionDto create(QuizQuestionCreateDto dto) {
		...
		String correct = (dto.getCorrectAnswer() == null || dto.getCorrectAnswer().isEmpty()) ? "A"
				: dto.getCorrectAnswer().toUpperCase().substring(0, 1);
		QuizQuestionEntity entity = QuizQuestionEntity.builder().lesson(lesson).question(dto.getQuestion())
				.optionA(dto.getOptionA()).optionB(dto.getOptionB()).optionC(dto.getOptionC()).optionD(dto.getOptionD())
				.correctAnswer(correct).build();
	...
	public void deleteById(Long id) {
		if (!quizQuestionRepository.existsById(id))
			throw new MyNotFoundException("Quiz sual tapılmadı: " + id);
	...
	private QuizQuestionDto toDto(QuizQuestionEntity e) {
		return QuizQuestionDto.builder().id(e.getId()).lessonId(e.getLesson().getId()).question(e.getQuestion())
				.optionA(e.getOptionA()).optionB(e.getOptionB()).optionC(e.getOptionC()).optionD(e.getOptionD())
				.correctAnswer(e.getCorrectAnswer()).build();
	}
}
