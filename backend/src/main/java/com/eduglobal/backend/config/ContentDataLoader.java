...
public class ContentDataLoader implements CommandLineRunner {
	...
	public void run(String... args) {
		...
		if (grammarLesson == null) {
			...
			grammarLesson = lessonRepository.save(LessonEntity.builder().title("To Be (Am/Is/Are)").content(content)
					.level(a1).category(Category.GRAMMAR).build());
		}

		if (quizQuestionRepository.findByLessonId(grammarLesson.getId()).isEmpty()) {
			quizQuestionRepository.save(QuizQuestionEntity.builder().lesson(grammarLesson).question("I ___ a student.")
					.optionA("am").optionB("is").optionC("are").optionD("be").correctAnswer("A").build());
